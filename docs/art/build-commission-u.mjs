// Build CODEX_COMMISSION_U.html + CODEX_MASTER_PROMPT_U.md from commission-u.mjs
// (refoundation v3: the two-class batch). Machine-checks the T stem↔manifest
// bijection against keen-manifest-u.json — exits 1 on drift.
//   node docs/art/build-commission-u.mjs [--refs <dir>]
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { cardsU, futureSectionsU, EGA, STYLE_PIXEL_V2, STYLE_CUTSCENE } from "./commission-u.mjs";
...
export const SLICES_U = {
  creatures2_sheet: ["walker2-0", "walker2-1", "hopper2-0", "hopper2-1", "flyer2-0", "flyer2-1", "thief2-0", "thief2-1", "cushion2-0", "cushion2-1", "cloud2-0", "cloud2-1"],
  boss2_sheet: ["boss2_head_idle", "boss2_head_tell", "boss2_card", "boss2_burst"],
  mapdecor_sheet: ["mdec_tree", "mdec_bush", "mdec_fence", "mdec_sign", "mdec_bridge", "mdec_rock", "mdec_flowers", "mdec_inkpool"],
};
const yielded = new Set();
for (const c of cardsU) for (const s of SLICES_U[c.stem] ?? [c.stem]) yielded.add(s);
const manifest = JSON.parse(fs.readFileSync(path.join(HERE, "keen-manifest-u.json"), "utf8"));
...
console.log(`bijection OK: ${allowed.size} manifest-T stems <-> ${cardsU.length} Batch U cards`);
...
<div class="card ${c.cls === "cutscene" || c.cls === "backdrop" ? "cutscene" : "pixel"}">
  ...
  <div class="usage">${esc(c.usage)}${SLICES_U[c.stem] ? `<br><b>Wird zerschnitten in:</b> ${SLICES_U[c.stem].join(", ")}` : ""}</div>
  <div class="drop">Speichern als: <code>~/Code/codex-art-lab/batch-u/${c.folder === "root" ? "" : c.folder + "/"}${c.file}</code></div>
  <details open><summary>Prompt (komplett kopieren → Codex)</summary><pre>${esc(c.prompt)}</pre></details>
</div>`;

const html = `<!-- GENERATED by build-commission-u.mjs — edit commission-u.mjs, not this file -->
<meta charset="utf-8">
<title>DomiGo · Codex-Kommission — Batch U (Neugründung: Cutscenes + gemalte Hintergründe + Gameplay v2)</title>
<style>
...
<h1>🖋 DomiGo — Die Codex-Kommission · Batch U (Runde 2: die Kohäsions-Runde)</h1>
<p><b>Für Koki.</b> ${cardsU.length} Generierungen → ${allowed.size} Spiel-Dateien. <b>NEU: zwei Bildklassen.</b> 🖼 <b>CUTSCENE/BACKDROP</b> = hochauflösend GEMALT (Bilderbuch-Qualität, keine Pixel — dein Verdikt: hochskalierte Pixel „sehen kaputt aus"). 👾 <b>GAMEPLAY</b> = echtes Pixel-Art in nativer Auflösung (Batch-S-Vertrag). Reihenfolge ist Gesetz: <b>Karte 1 = der gemalte Style-Key</b>, dein Blick drauf, dann der Rest.</p>

<h2>§0 · So gehst du vor</h2>
<div class="step">1 · Öffne Codex (Agent-Modus). Am einfachsten: den kompletten <code>CODEX_MASTER_PROMPT_U.md</code> einfügen — Codex arbeitet alle ${cardsU.length} Karten selbstständig ab. Alternativ Karte für Karte aus dieser Datei.</div>
<div class="step">2 · Sandbox-Gesetz unverändert: Codex schreibt AUSSCHLIESSLICH nach <code>~/Code/codex-art-lab/batch-u/</code> — nie ins Repo.</div>
...
<div class="step">5 · Danach (Fable): QA-Gate + Slicing + Sync gegen <code>keen-manifest-u.json</code>. Fehlende Bilder sind nie ein Problem — prozedurale Grafik bleibt als Fallback.</div>
...
<h2>§2 · Batch U — die Karten</h2>
${cardsU.map(cardHtml).join("")}

<h2>§3 · Spätere Batches</h2>
${futureSectionsU.map((s) => `<div class="card"><div class="head"><code>${s.title}</code></div><div class="usage">${esc(s.note)}</div></div>`).join("")}
`;
fs.writeFileSync(path.join(HERE, "CODEX_COMMISSION_U.html"), html);
console.log(`CODEX_COMMISSION_U.html written (${(html.length / 1024).toFixed(0)} KB, ${refs.length} embedded references)`);

// ── THE MASTER PROMPT T (autonomous mode) ──
const stripStyles = (p) => p.replace(STYLE_CUTSCENE, "").replace(STYLE_PIXEL_V2, "").replace(/^\s*\n/, "");
const master = `# MISSION: produce the complete image set for "DomiGo — Batch U" (${cardsU.length} images, TWO style classes)
...
SAVE TO: ~/Code/codex-art-lab/batch-u/${c.folder === "root" ? "" : c.folder + "/"}${c.file}
SIZE: ${c.size}
...
`;
fs.writeFileSync(path.join(HERE, "CODEX_MASTER_PROMPT_U.md"), master);
console.log(`CODEX_MASTER_PROMPT_U.md written (${(master.length / 1024).toFixed(0)} KB, ${cardsU.length} cards)`);
