...
Dieses Dokument ist ein WERKZEUG, kein Essay. Es beantwortet drei Fragen, und nur die:

1. **Was weiß das Kind zu welchem Zeitpunkt?** (§1) — keine Zeile darf Wissen voraussetzen,
   das erst später kommt.
2. **Womit ist eine Zeile gedeckt?** (§4) — jede Behauptung nennt die Weltsache, die sie
   beweist. Eine Zeile ohne Deckung wird umgeschrieben oder gefiled. Nie beides nicht.
3. **Wie klingt das Kapitel?** (§5) — der Ton-Kanon, den Session G1 beim Neuautoren der
   Karten-Batterien anwendet.

**Das eine Gesetz über allem:** *Die Copy behauptet nur, was das Kind sieht.* Was die Welt
nicht zeigt, sagt keine Karte — es wird als Weltsache bestellt (Debt-Register) oder es
existiert nicht.

---

## §1 · Was das Kind weiß, wann (das Wissens-Ledger)

Jede Zeile im Kapitel wird gegen diese Tabelle geprüft: sie darf nur Begriffe benutzen, die
in ihrer Zeile oder DARÜBER schon eingeführt sind.
...
ebenfalls gefangen ist · dass es fünfzehn Klassenkinder sind. Die Cloak-Regel (doc 31 §6,
Unmask = ch15) ist eine Copy-Regel, keine Design-Notiz.

---

## §2 · Ein Fluch, drei Zustände — je ein Wort, überall dasselbe

Kokis Befund C6 („‚Dinge' ist ein Platzhalter, der schleunigst ausgetauscht werden muss…
und die zwei Familien klar trennen"). Der wichtige Teil, den eine erste Fassung dieses
Dokuments falsch hatte: **es sind keine Familien von WESEN, sondern Zustände — und ein
Wesen kann in zweien gleichzeitig sein.**
...
| Käfig | Phase | Insasse | Grammatik | Wie die Copy ihn nennt |
|---|---|---|---|---|
| 1 | p1 | Musikanlage | Ding | *die Musikanlage* |
| 2 | p2 | Tablet | Ding | *das Tablet* |
| 3 | p2 | **Merle** | Person | *Merle* |
...
## §3 · Der Bogen — Szene für Szene

Fünf Räume, ein Gefälle: **allein → Werkzeug → Mensch → Können → Prüfung.** Jede Phase hat
ihre eigene Raum-Idee (aus den Dossiers, unverändert); der Bogen ist, was sie verbindet.

**0 · Die Auftrags-Karte.** Der einzige Moment, in dem das Buch selbst spricht. Sie nennt
den Verlust (Titel), die Ursache (der Tinten-Schatten), die Geste (freimachen — nicht bloß
benennen) und die fünf Versprechen mit ihren Zahlen. Danach erzählt nur noch die Welt.

**1 · p1 · Die Eingangshalle — GESTRANDET-HOCH.** Alles, was dem Kind gehört, ist beim
Sturz ins Buch nach oben gestrandet: das Buch auf dem Stapel, die Tasche auf dem Spind, die
Musikanlage im Käfig über dem Tor. Die Halle ist der Lehr-Raum: hier lernt das Kind alle
drei Gesten (restaurieren · begegnen · befreien) an je einem Beispiel. Der Käfig über dem
Tor ist das erste sichtbare Ziel, das noch nicht erreichbar ist.
...
Ding des Kapitels, und genau deshalb komisch.

**4 · p9 · Die Kleckskammer (optional).** Klecks verkauft eine Tür für 8 Buchstaben.
Drinnen: ein Trail, der ein Wort schreibt — **SCHOOL THINGS** —, und eine Uhr aus
trocknender Tinte. Die Kammer gibt nichts als Buchstaben und Angeberei. Genau das ist ihre
Rolle: der einzige Ort des Kapitels ohne Aufgabe.
...
**Die Türen — der Faden, der alle Räume näht.** Jede Phase endet an einer Tür, und jede
...
## §3a · Kreuztabelle — welcher Beat welche Karten hat

Damit eine Autor-Session nicht raten muss, welche Karten zu welchem Story-Beat gehören
(die Bindung läuft über `use` + `phases` + `skins`, nicht über den Namen):

| Beat (§1) | `use` | Karten-IDs | Anz. |
|---|---|---|---|
...
| 13 · der Sieg | finale | `fin.t1` | 1 |
| 14 · Käfig #5 | rescue | `rsc.picture.r1` | 1 |

Summe 69. Beats 0, 1, 11, 15, 16 tragen keine Karten — sie sind Zeremonien des Shells
(Auftrags-Karte, Käfig-Lehrkarte, Befreiungs-Karte, Bilanz-Seite, Tür hinaus).

---

## §4 · Das Behauptungs-Ledger — jede Zeile und ihr Beleg

So wird gearbeitet: **Behauptung → Weltsache → wie belegt.** Eine Zeile, deren Weltsache
leer ist, wird umgeschrieben (Spalte »Konsequenz«) oder als Weltsache bestellt.

### 4a · Gedeckte Behauptungen — diese Zeilen dürfen so etwas sagen
...
### 4b · UNGEDECKTE Behauptungen — die Befunde dieser Session

| Behauptung (heute im Spiel) | Was die Welt wirklich zeigt | Konsequenz |
|---|---|---|
...
| „Der Käfig **wackelt** — drinnen glimmt der Lautsprecher" | Der Wackel-Zustand ist in ch01 nur über einen Faust-Treffer erreichbar, also nie. Der Insasse hat keine Kunst (Codex AQ läuft). | **Die BEWEGUNGS-Behauptung fällt** — sie ist heute schlicht falsch. Der Insassen-NAME bleibt: er ist der einzige faire Hinweis, und Deutsch-nennt-Englisch-antwortet ist das Prinzip jeder Karte im Kapitel. Wackeln (F1) und Insassen-Kunst (Codex AQ) als Schuld eingetragen — dann wird die Zeile ganz wahr |
| „Wesen befreit" (Bilanz-Zeile) | Gezählt werden vier Käfige mit Gegenständen. | Zeile benennt die Gefangenen, nicht „Wesen" |
| „Alle 12 Buchstaben" (Kleckskammer) | Der Trail schreibt SCHOOL THINGS. | Die Fänge werden als Wort ausgelegt |

---

## §5 · Ton-Kanon (verbindlich für Session G1 und jede spätere Karten-Welle)

**Die Stimme.** Ein Erzähler, der neben dem Kind steht und auf Dinge zeigt. Er staunt nicht
für das Kind mit, er erklärt das Spiel nicht, und er redet nie über sich selbst oder über
das Buch als Produkt (No-Meta-Talk). Zwei Sätze pro Karte: **was zu sehen ist** und **was zu
tun ist.**

**Die Karten-Anatomie** — welches Feld welche Arbeit tut. Wer neu autort, füllt alle:

| Feld | Was es tut | Regel |
|---|---|---|
...
**So klingt richtig.** Die Zeilen unten sind die, die den Ton HEUTE schon treffen — sie
stehen als Maßstab hier, nicht als Vorlage zum Abschreiben. Je Karten-Art eine:
...
| quickfire / wheel | `qf.moths.w1` | „Ein Zahlen-Falter surrt dir um die Nase." / „Dreh das Rad auf seine Zahl!" |
| quickfire / choice (**ohne Wesen**) | `qf.free.c2` | *(kein showsDe — niemand fragt)* / „Du bekommst dein Buch zurück. Was sagst du?" |
| door / choice | `door.d7` | „Die Tür steht offen, es zieht eiskalt herein." / „Sag ihr, sie soll ZUGEHEN!" |
| rescue / choice (Käfig) | `rsc.chair.r1` | „Hinter den Gittern sitzt still ein grauer Stuhl." / „Sag, was du siehst — dann springt der Käfig auf!" |
| rescue / choice (Merle) | `awk.merle.r2` | „Merle steht mit beiden Schuhen oben auf dem Tisch." / „Der Tisch ist kein Boden — sag es ihr!" |
...
1. **Die Auszahlungs-Klausel ist optional, aber nie erfunden.** „…— dann macht er Platz!"
   ...
   dem Auftrag. Lieber ein kurzer Satz als ein erfundenes Versprechen.
...
- **≤ 56 Zeichen** — und zwar **genau** für `showsDe`, `storyDe`, `colourAskDe`
  ...
  Regel-Seiten-Merksatz, für `whyDe` und für jeden Hinweis auf der Auftrags-Karte.
  ⚠ **Die `hints`-Felder sind heute von KEINEM Gate gedeckelt** — `enc.ranzen.q1`
  trägt 59 Zeichen. Halte sie trotzdem in einer Zeile; die Lücke ist gefiled.
...
- **Kein Farbwort über einem entfärbten Wesen**, solange es grau ist (Desaturations-Gesetz).
- **Keine Eile-Wörter ohne Uhr** und keine Ruhe-Wörter auf einer Karte mit Uhr.
- **Kein englisches Antwort-Wort im eigenen `promptEn` oder `storyDe`** (Giveaway-Gesetz).
  Das deutsche Wort DARF in `showsDe` stehen — genau darauf beruht das ganze Kapitel:
  Deutsch rahmt, Englisch antwortet („Ein dickes Buch lehnt blass an der Wand." → *It's a
  book*). Wer dem Käfig seinen Insassen wegnähme, machte die Karte unlösbar.
- **Jedes englische Wort** steht im Korpus `content/corpus/units/g1-u01/`
  (`wordbank.json`, `vocab.json`, `grammar.json`) — geprüft gegen
  `docs/design/g1/grounding/u01-lexicon.json`.

**Typografie.** Gesprochenes steht in **„…“** — eine Konvention im ganzen Kapitel. Innerhalb
von JS/TS/JSON-Strings nie ein ASCII-`"` als Schluss-Zeichen (es beendet den String);
wo das nicht sicher geht, **»…«**.

**Zahlen.** Keine Zahl wird getippt, die die Welt auch zählen kann. Steht eine Zahl in einer
Zeile, kommt sie aus den Daten — sonst ist sie irgendwann falsch.

---

## §Quellen

doc 44 §1 (Kanon), §2.2 (Freien schlägt Benennen), §2.3 (Käfig-Gesetz), §2.6
(Auftrags-Bildschirm), §4 ch01 · doc 45 §C1–C11 (Copy-Schulden), §D1–D9 (Story-Programm) ·
`ch01-dossiers-v2/README.md` (§Kanon, §Tor-Antworten 2026-08-11, §Käfig-Zensus) + p1/p2/p3/
p9/arena §1 · Kokis Replay 2026-08-11 (07:24–07:34) · Kokis Tor-Antworten in Session C1
(Titel = den Verlust benennen · der Verursacher heißt »der Tinten-Schatten«).
