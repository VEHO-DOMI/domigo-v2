...
> **⚠ 2026-06-30 status — most of Track C is now DONE.** Since this doc was written, **three of the four grade games shipped to `main`**: the **G1 "Lost Pages" overworld** (now a full **15-zone uplift** — theming engine + all 15 units authored + comprehension + hub board + art pipeline, PRs #72–#78), the **G2 "The Wrong Name"** detective (15 chapters + its **complete Part-6 pedagogy**, PRs #40–#60), and **G3 "FOURTEEN"** (14 episodes + season board + comprehension + art, PRs #61–#71), plus a brand design system across all 4 grades (#62/#63). **§4 below (Track C kickoff) is DONE for G1/G2/G3.** Still open from this doc: **§1 B2b**, **§2 content waves**, **§3 TTS**. Still ahead: **G4 "Syntaxia"** (the last game) + **Track D** (migration cutover + bulletproof-beta + PWA + cheap-Android perf) before go-live. **The live source of truth is [`../STATUS_AND_ROADMAP.md`](../STATUS_AND_ROADMAP.md).**

> **How to use this doc.** DomiGo v2's full P1 learning track + 3 of 4 games are built and verified (see `STATUS_AND_ROADMAP.md`). The sections below remain self-contained passovers — a fresh session can pick up **one** cold. Work in `~/Code/domigo-v2`. **Read `08_design_principles.md` (the guardrails) before any change.** Pick by priority: **G4 Syntaxia** (the last game — mirror the G2/G3 build; full design in `10_game_layer.md`) · content waves (large, multi-session) · B2b (small, closes the Mock-Test loop) · TTS (small) · Track D (gates go-live).
...
## 4. Track C — the grade games  ✅ G1 + G2 + G3 SHIPPED · ❌ G4 remains (2026-06-30)
...
> **Status (2026-06-30).** **Three of four games are DONE on `main`:** **G1 "Lost Pages"** (full 15-zone uplift — theming + all 15 units + comprehension + hub board + art, #72–#78), **G2 "The Wrong Name"** (15 ch + complete Part-6 pedagogy, #40–#60), **G3 "FOURTEEN"** (14 episodes + board + comprehension + art, #61–#71). **The one remaining game is G4 "Syntaxia"** (not started — the biggest scope: portal-worlds gated by grammar mastery, real branching proven dead-end-free by VS-5, a hint-economy party, optional Three.js set-piece). Build it the way G2/G3 were built: G0 story+map contracts → a vertical slice (GO/NO-GO with Koki) → act/world content waves → enrichment (comprehension `.ci.` + hub board + art pipeline). The seams below (the 9 Laws, `getDueRefs` encounters, the task-ui overlay, `/api/attempts`, `game_saves`, the `story@1`/`map@1` pipeline + VS-1…VS-12) are exactly what G4 reuses — proven across the first three games.
