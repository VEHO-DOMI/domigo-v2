...
- **Dev server:** preview name is **`domigo-web`** (global; do NOT add a repo `.claude/launch.json`). Dev identity via `apps/web/.env.local` (`DEV_USER_ID`/`DEV_CLASS_ID` + dev Neon branch). `/play` was kept **out of the middleware matcher** (page self-guards) so dev-login would not break; since welle-076 (2026-09-26) exactly `/play/:grade` is in it (a signed-out child returns to its year after konto), and `lib/dev-durchlass.ts` lets both `DEV_USER_ID` and `DEV_TEACHER_ID` pass there — deeper `/play` paths stay out. Zsh: `UID`/`EUID` are reserved — use another var in loops.
