...
3. **Keys touched ⇒ blind-solve.** Any change that creates or edits an answer key (new items, key rewrites, pool patches) passes an adversarial blind-solve **graded through `@domigo/engine`** — never string-compare, never a second brain, not even in audit. Until the A-4 harness lands, the floor is the dev-server spot-check: POST every touched key through `/api/attempts` and require `tier:"correct"`.
...
- **R1 — article variants:** an `enToDe` pool whose gloss is a German noun carries the bare form AND the der/die/das form as full-tier answers.
