...
Exit criteria: `pnpm content audit-variants` prints `scanned: <N> · critical: <C> · advisory: <A>` + writes both artifacts (exit 0 always — audit, not gate); fixture tests (known-missing article / asymmetric contraction / bad pipe-count each yield exactly their row; clean fixture yields zero). Wave exit: re-run prints `critical: 0` on all 57 units. Koki-gate **K-4**: 20-item calibration of the first report (~30min; his pedagogy decides e.g. bare-noun-without-article = full or partial). **✅ DECIDED 2026-07-11** — bare noun sufficient, correct article never wrong (both full-tier); general principle "a correct answer with a correct addition is still correct" (covers R2 ambiguous-`'s` too). Recorded in handover/17 §2 R1. Gate-presentation lesson: policy calls are put to Koki as ONE plain question + 2–3 examples + a recommendation — the raw worklist was harder to review than the decision deserved.
...
**A-4 · E-4 blind-solve harness** (2d harness + ~0.5d/grade runs, off-CI) — **✅ SHIPPED (harness + dry audit; live runs pending API key)**
_As built (deliberate deviations from the spec below): CLI is `scripts/audit/blind-solve.ts` (node ≥22 runs TS directly — no `.mjs` wrapper); the pure core (frames, task-ui shuffle port, triage, prompt assembly) lives in `packages/content-pipeline/src/blind-solve.ts` so it is fixture-tested under the package suite; model default is `claude-opus-4-8`, NOT cheap-tier — a weak solver's confident-wrong answers manufacture false class-(a) findings and burn Koki gate time, while cost stays contained via `--effort low` + the response cache (`--model claude-haiku-4-5` remains available for a comparison sweep during K-4/K-8 calibration); v1 frame scope is vocab-carrier + the six text formats + mc/context-picker — matching/sort/sentence-building/anagram are counted as skipped because their affordances (tiles/chips) derive from the answer itself and cannot be framed without leaking; `--no-llm` is not just plumbing proof: it feeds EVERY authored full answer back through the frame's input shape and the real engine (key-solvability invariant) — first run: 5,237 items across all 4 grades, 0 key-defects._
...
_[2026-07-14: **STAGE 1 BUILT** (branch `feat/b2-overworld`) against the Koki-gated design doc `handover/22` — steps 1, 3 (map.json + the four exemplar themes; the other 11 themes are stage 2, their zones ship `render:null`), 4, 5, 6, plus the `dgh-ink` floor-plan hub and the VS-18 map-integrity validator; step 7 (art manifests/prompt libraries) is the separate ART-2 lane. NO release.json yet; stage 2 waits on Koki's LOOK checkpoint (/play/2 + z01)._
...
### IV.3b · WS-LOOK — The Look & Feel program (Koki directive 2026-07-11: "really go all out on UI/UX in the game design of all grades — push the boundaries of what's possible")

_Ground truth (audited 2026-07-11): the ENTIRE visual layer is procedural placeholder today — G1 renders `art-gen`'s painted tiles (no image-loading path exists in `OverworldScene` at all), G2/G3/G4 render procedural SVG fallbacks; the only shipped assets are 8 G2 character *reference* images (8/126 of the G2 manifest); the iCloud drop folder `generated-images/g1-school/` is EMPTY. The pipeline is fully designed (FireRed 48px bar, only-present resolver, stem-suffix sync) but three load-bearing pieces are unbuilt. The PASSOVER note claiming #102 is unmerged is STALE — #102 merged 2026-07-09, so the crafted-48px procedural fallback is already live. Two art languages, deliberate: **Pokémon FireRed/LeafGreen pixel** for the overworlds (G1 now, G2's school overworld next — shared tile language), **warm ligne-claire storybook** for the DOM games (G2 detective / G3 FOURTEEN / G4 trip)._

**The ambition bar:** the games should look and feel AUTHORED, not assembled — FireRed-quality interiors, tiles readable at 48px, motion that breathes. Whatever imagery is needed, Koki generates (his explicit offer); whatever the engine must do to present it beautifully, we build — **inside the frozen contracts** (15×11 grid, 48px tiles, save shape v1, Phaser ≤400KB gz; juice must never break Law 9's ≥30fps floor).

**LOOK-0 · Cast-canon + prompt-library truth pass** (0.5d, Fable-authored register judgment; Koki confirms cast) — the G1 prompt library invents a player mascot ("Domi") and a kid-Finn that CONTRADICT canon (Finn = floating paper-and-ink book-guide; Pixel = book-cat; and the G1-N bible adds Jona for late zones). Re-key `CHAR_SPECS` in `docs/art/G1_SCHOOL_IMAGE_PROMPTS.html` to canon; the player stays the per-student seeded procedural sprite UNTIL a deliberate decision replaces it (a generated sheet would erase per-student identity — flag as its own Koki call). Fix the stale #102 line in the iCloud PASSOVER. Exit: library canon-true; HOLD on character generation lifted.

**LOOK-1 · Image-first tileset resolver** (~1d, Opus) — PULLED FORWARD out of B-2 step (2), because it unlocks G1 real art WITHOUT waiting for the G2 overworld build: server `resolveTileArt(storyId, generator)` in `apps/web/lib/` over `apps/web/public/art/<ns>/` (only-present discipline, mirrors `story-art.ts`); Phaser `preload()` queues the PNGs that exist; `paintTileset` stays the per-kind fallback; `finn_down.png` wires the NPC texture (kills the Finn-as-desk bug). Exit: with an empty art dir the game is pixel-identical to today (screenshot diff); with one test PNG dropped, that tile renders it (screenshot in PR); bundle gate green.

**LOOK-2 · Sync + prep pipeline generalization** (0.5–1d, Opus) — `sync-art.mjs --lib/--dest` (currently g2-only), NEW `g1-art-files.json` manifest (35 school stems + per-zone growth), NEW `prep-art.mjs` (downsample to 48px, palette-quantize, dimension/transparency verify — the QA step of the art loop, mechanized). Exit: dry-run on the empty drop folder lists expected-but-missing stems as a worklist; a fixture PNG round-trips drop→public with correct name+size.

**LOOK-3 · Generation waves (Koki generates; sessions integrate via the art loop)** — order per doctrine: `_style_key.png` FIRST (fed as reference into every subsequent generation) → school-zone tiles/props (the existing 35-prompt library) → per-zone prompt libraries for the other 14 G1 zones (authored as needed, same STYLE constant) → character sheets ONLY after LOOK-0. Every integration PR: prep-art QA → sync → **in-game screenshot compared against the style key** (art loop; 10A). G2's school overworld (B-2) then inherits the whole pipeline + tile language.

**LOOK-4 · DOM-game art waves** (rides WS-C chapter waves) — wire the 8 G2 `*_ref` images as actual in-game portraits via the existing `story-art.ts` resolver; grow the G2 126-stem manifest; G3/G4 ligne-claire panel waves through their `art.json` resolvers. Exit per wave: portraits render in-chapter (screenshot), fallback intact for missing stems.

**LOOK-5 · Motion & presence** (rides GX; specs live in v1 §III.5 — do not duplicate) — the engine-side "alive" work that needs no assets: animated encounter-✦ shimmer, subtle water/plant idle frames in `paintTileset`/loaded tiles, camera ease-in on zone entry, NPC idle bob, per-chapter palette tinting (dusk for sad beats — pairs with B12 dark canvas), boot ceremonies (B1) and hub juice (B5). Each behind the perf floor; `check:bundle` + a throttled-fps spot check per PR.

Sequencing: LOOK-1 → LOOK-2 are immediately Opus-executable (before or parallel to B-0); LOOK-0 needs one Koki confirm; LOOK-3 starts the moment 1+2 land and never blocks code lanes; LOOK-4/5 ride existing waves. **Koki's first generation task once LOOK-1/2 land: the style key, then the 35 school prompts** (drop folder: iCloud `DomiGo v2 Art Prompts/generated-images/g1-school/`, filenames ending in the stem).
...
**C-1 · Checkup assignment mode — migration 0008 (two additive columns)** (4d)
**✅ BUILT 2026-07-13 (Fable, PR pending).** As-built deltas vs. this block, all doc-21-governed: migration shipped as **0008_checkup_config** — TWO nullable jsonb columns (`assignment_sections.section_config` + `assignments.display_config` for §4b feedback visibility; the doc's "0009" predated the drizzle sequence) · reserved items **EXCLUDED** (doc 21 §5.1/§8-⑤ supersedes this block's "reserved-first") · picture-mc **deferred** (§8-②, +points fallback keeps /20) · checkups are **points-only** (§8-③ — note stays null; scorePct stores points/20) · runner = NEW one-page `CheckupRunner.tsx` (paper header ___/20, per-section ___/n) rather than an `AssignmentRunner` layout branch · `verify-checkup.mjs` covers the DB-free tiers (compose Σ=20 corpus-wide, deterministic seeds, 1,140-item self-grade sweep in the SAME pool the checkup asks, hand-computed 16/20 half-credit fixture, tamper checks); the teacher→student→wall E2E leg needs a DB and rides the deploy checklist. Gate ①–⑥ answers recorded in doc 21 §8 + addendum.
...
Files: NEW `apps/web/lib/checkup.ts` (`GRADE_STRUCTURES` presets per grade — **extracted: `docs/handover/21_checkup_presets.md` (F-1, 2026-07-13) is the binding preset spec, incl. the reserved-EXCLUDED correction to this block's "reserved-first"** — from the efl-checkup-generator framework + Koki's real class checkups — + `composeCheckup(unit, reservedItems)` summing to exactly 20, reserved-first) · builder "Checkup" template in `AssignmentBuilder.tsx` · API accepts mode+config with server-side 20-point-sum validation · one-page layout in `AssignmentRunner.tsx` · first-letter mask prop in `packages/task-ui` · /20 points-weighted scoring extension (pure, boundary-tested).
...
13. P-1 identity ✅ (#141 migration 0006 · #142 class CRUD) → P-2 roster ✅ (#143 stacked-merge miss, re-landed #144, migration 0007 nickname guard) → C-1 checkups ✅ BUILT (2026-07-13, migration 0008, PR pending — apply 0008 in Neon before merge)
