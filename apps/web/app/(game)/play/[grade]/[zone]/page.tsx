...
export default async function ZonePage({ params, searchParams }: { params: Promise<{ grade: string; zone: string }>; searchParams: Promise<{ from?: string }> }) {
  ...
  if (gameType === "detective") {
    ...
    const dueRefs = await getDueRefs(getDb(), acting.classScope, acting.userId, acting.classId, { kind: "unit", slug }, 3).catch(() => []);
  ...
  const due = await getDueRefs(getDb(), acting.classScope, acting.userId, acting.classId, { kind: "unit", slug }, Math.max(8, encounterCount * 2)).catch(() => []);
