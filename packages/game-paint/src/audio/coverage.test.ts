...
describe("Abdeckung: jede Ereignis-Art des Spiels hat genau einen Zustand", () => {
  const cases = [
    { file: "sim.ts", name: "SimEvent", table: SIM_REACTIONS as Readonly<Record<string, unknown>>, expected: 27 /* R1b: seven observable zoo/fist events +  15 + `cloth` (R5-W5 · G4, 18.08.) + `gate` (R5-W7 · S3, D-372) + die drei der Bilge (L3-M-a · E5) */ },
    { file: "player.ts", name: "PlayerEvent", table: PLAYER_REACTIONS as Readonly<Record<string, unknown>>, expected: 8 },
    { file: "entities.ts", name: "EntityEvent", table: ENTITY_REACTIONS as Readonly<Record<string, unknown>>, expected: 22 /* R1b: five zoo/shuttle events +  + `pumpHit` (L3-M-a · E5) */ },
  ];
  ...
  it("zusammen sind es 57 (45 + zwölf Zoo-Ereignisse) — und jede Reaktion ist genau eines von drei Dingen", () => {
